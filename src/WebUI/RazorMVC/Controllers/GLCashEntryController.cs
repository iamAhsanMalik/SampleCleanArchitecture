...
{
    public class GLCashEntryController : Controller
    {
        ...
        // GET: CashEntry
        public ActionResult CashEntry(int? id)
        {
            ...
            return View("~/Views/Postings/CashEntry.cshtml", postingViewModel);
        }
        ...
        [HttpPost]
        public ActionResult ProceedCashEntry(PostingViewModel pPostingModel, IEnumerable<HttpPostedFileBase> files)
        {
            string HostName = System.Web.HttpContext.Current.Request.Url.Host;
            pPostingModel.isPosted = false;
            pPostingModel = PostingUtils.ProceedCashEntry(pPostingModel);
            if (pPostingModel.isPosted)
            {
                ...
                return RedirectToAction("ShowMessage", "GLCashEntry", new { cpUserRefNo = Security.EncryptQueryString(Common.toString(pPostingModel.GLReferenceID)) });
            }
            else
            {
                pPostingModel.ErrMessage = Common.GetAlertMessage(1, pPostingModel.ErrMessage);
            }
            return View("~/Views/Postings/CashEntry.cshtml", pPostingModel);
        }
